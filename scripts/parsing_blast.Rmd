...
write_rds(taxonomy.now, here("input/taxonomy.rds"))
```
...
taxonomy.now <- read_rds(here("input","taxonomy.rds"))

#blast_output <- "../pipeline_output/hash_key_221103.txt"
blast_output <- "hash_key_221103.txt"

BLAST_results <- read_table(here("output", blast_output), col_names = c("qseqid", "sseqid",  "pident", "length" ,"mismatch", "gapopen" ,"qstart","qend", "sstart", "send" ,"evalue" ,"bitscore", "staxid", "qlen"))
...
write_rds(all.lineages, here( "output" , glue::glue("lineages_{Sys.Date()}.rds")))
...
all.lineages.list %>% discard(function(x) is.na(x[[1]]) ) ->  all.lineages.list.good
  ...
  bind_rows(.id = "taxID") %>%
    ...
    all.lineages.long %>% mutate (taxID = as.numeric(taxID)) %>%
      write_csv( here("output","taxonomies_matches.csv"))
```
...
```{r}
all.lineages.long <- read_csv(here("output","taxonomies_matches.csv"))
...
finalids %>%
  ...
  write_csv(here("output","hash_classified_95_100.csv"))
```

...
sample.map <- read_csv(here("raw_data", "metadata.csv"))
...
saveRDS(tosave, file = here("raw_data","all.errors.rds"))
rm(tosave)
 gc()
#
...
write_rds(derepF1s, here("raw_data","derepF1s.rds"))
write_rds(derepF2s, here("raw_data","derepF2s.rds"))
...
derepR1s <- derepFastq(filtR1s_good, verbose=TRUE)
 ...
 write_rds(derepR1s, here("raw_data","derepR1s.rds"))

rm(derepR1s)
gc()

derepR2s <- derepFastq(filtR2s_good, verbose=TRUE)
write_rds(derepR2s, here("raw_data","derepR2s.rds"))
rm(derepR2s)
gc()
...
gc()
derepF1s <- read_rds(here("raw_data","derepF1s.rds"))
dadaF1s <- dada(derepF1s, err = errF1, multithread = TRUE)
rm (derepF1s)
gc()

derepF2s <- read_rds(here("raw_data","derepF2s.rds"))
dadaF2s <- dada(derepF2s, err = errF2, multithread = TRUE)
rm (derepF2s)
gc()

derepR1s <- read_rds(here("raw_data","derepR1s.rds"))
dadaR1s <- dada(derepR1s, err = errR1, multithread = TRUE)
rm(derepR1s)
gc()

derepR2s <- read_rds(here("raw_data","derepR2s.rds"))
...
```{r saving to disk}
   ...
   saveRDS(list( dadaF1s, dadaF2s, dadaR1s,dadaR2s), file = here("raw_data","tosave.rds"))
